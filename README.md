...
`python bin/fit_and_val.py --config <your/config/file.yaml>`  
or  
`python bin/main.py fit --config <your/config/file.yaml>`

See [config documentation](config/configs.md) for more details on how to read and create config files.

## Inference

1. If you have the algae mask for a specific date:
```bash
python bin/predict.py --ckpt_path <path/to/checkpoit.ckpt> -i /path/to/your/algae/mask.npz -o <output/path.png>
```

2. If you only have the raw netcdf:
```bash
python bin/evaluate/2_predict_tests_samples.py --ckpt_dir /path/to/your/model --nc_dir /path/to/your/raw/netcdf
```

To compute prediction over all tests samples (sent by T. Guinaldo, stored in `/scratch/shared/sargasses/tests/netcdf/`):
```bash
python bin/evaluate/2_predict_tests_samples.py --ckpt_dir /path/to/your/model
```